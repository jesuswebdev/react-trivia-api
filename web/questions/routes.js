      ...
      options: {
        ...
        validate: {
          payload: Joi.object({
            ...
            tags: Joi.array().items(Joi.string().trim().min(4)).optional(),
            did_you_know: Joi.string().min(8).optional(),
            link: Joi.string().regex(/^http/).uri().min(10).optional()
          }),
          query: false
        }
      }
            ...
            options: Joi.array()
              ...
              .items(
                Joi.object({
                  text: Joi.string().trim().required(),
                  correct: Joi.boolean().required(),
                  option_id: Joi.number().integer().min(0).max(3)
                }).options({ stripUnknown: true })
              ),
            category: Joi.string().alphanum().trim().length(24),
            state: Joi.string().allow("approved", "rejected", "pending"),
            did_you_know: Joi.string().min(8).optional(),
