...
const { shuffleArray, getModel, castToObjectId } = require("../../utils");
...
exports.create = async (req, h) => {
  ...
  try {
    const { questions: gameQuestions } = await GameModel.findById(
      castToObjectId(gameToken.game_id)
  ...
  try {
    createdGame = await GameModel.findByIdAndUpdate(
      castToObjectId(gameToken.game_id),
      {
        $set: {
    ...
    for (const question of req.payload.questions) {
...
exports.find = async (req, h) => {
  ...
  try {
    foundGames = await GameModel.find({}).populate("questions.question").lean();
  } catch (error) {
    return Boom.internal();
  }
...
exports.findById = async (req, h) => {
  ...
  try {
    foundGame = await GameModel.findById(castToObjectId(req.params.id))
...
exports.answer = async (req, h) => {
  ...
  try {
    ...
    const game = await GameModel.findById(castToObjectId(req.params.gameId));
    if (
      (game.current_question || {}).toString() !== token.current_question.id
    ) {
      return Boom.badData();
    }
    ...
    let updatedGame = {
      ...
      remaining_attempts: game.remaining_attempts + (answerIsCorrect ? 0 : -1),
    ...
    let newToken = await Iron.seal(
      {
        ...
        current_question: {
          id: newQuestion._id,
          correct_answer_id: newQuestion.options.find(({ correct }) => correct)
            .option_id
        },
