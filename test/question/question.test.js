...
//       options.payload.options = options.payload.options.concat({
//         title: "hola mundo",
//         correct_answer: false
//       });
//       const { statusCode } = await server.inject(options);
...
//       options.payload.options = options.payload.options.map(o => {
//         return { ...o, correct_answer: true };
//       });
//       const { statusCode } = await server.inject(options);
//       expect(statusCode).to.equal(400);
//     });
...
//       expect(result)
//         .to.be.an.object()
//         .and.to.contain(["results", "results_count"]);
...
//     test("fails when options array is more than 4", async () => {
//       options.payload.options = options.payload.options.push({
//         title: "hola mundo",
//         correct_answer: false
//       });
//       const { statusCode } = await server.inject(options);
